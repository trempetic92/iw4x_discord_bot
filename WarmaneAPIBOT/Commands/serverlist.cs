...
{
    class ServerListCommand : ModuleBase<SocketCommandContext>
    {
        ...
        {
            await Context.Channel.SendMessageAsync("```css\nPlease wait until all the servers with players have been processed. Thank you!```");
                ...
                var LastTask = new Task<Task>(async () =>
                {
                    ...
                    Server model = ServerAPIManager.GetServerInfo(element);
                        ...
                        if (model.players.Count > 0)
                        {
                            ...
                            sw.WriteLine(string.Format("{0,-30}", $"[{element}]", 30) + string.Format("{0,-60}", $"[{model.status.sv_hostname}]", 30) + string.Format("{0,-10}", $"[{model.players.Count}/{model.status.sv_maxclients}]") + $"[{Country.country}]");
