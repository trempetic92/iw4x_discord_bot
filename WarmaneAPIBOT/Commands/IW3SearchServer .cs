...
{
    class IW3SearchServerCommand : ModuleBase<SocketCommandContext>
    {
        [Command("iw3search")]
        [Alias("iw3search")]
        public async Task Handle(string servername)
        {
            ...
            await dm.SendMessageAsync($"```css\nPlease wait until all COD4 servers with RaidMax Admin Tool that contain word {servername} are listed. Thank you!```");
