...
{
    class T6SearchServerCommand : ModuleBase<SocketCommandContext>
    {
        [Command("t6search")]
        [Alias("t6search")]
        public async Task Handle(string servername)
        {
            ...
            await dm.SendMessageAsync($"```css\nPlease wait until all BO2 servers with RaidMax Admin Tool that contain word {servername} are listed. Thank you!```");
